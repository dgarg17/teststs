    ...
    setValue: function(value) {
        this.fields.each(function(f) {
			if (value[f.rawFieldName]) {
				...
				// check for a nested node and property
				if (f.rawFieldName.indexOf("/") > -1) {
					var segments = f.rawFieldName.split("/");
					var nodeName = segments[0];
					var propName = segments[1];
					if (value[nodeName]) {
						var nestedNode = value[nodeName];
						if (nestedNode[propName]) {
							f.setValue(nestedNode[propName]);
						}
